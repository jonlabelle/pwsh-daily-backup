...
- **NEW PARAMETER**: `-Force` for `New-DailyBackup`
...
- **NEW PARAMETER**: `-FileMode` for `New-DailyBackup`
  - `Individual`: Each file/directory gets its own ZIP archive
  - `Combined`: All files/directories combined into a single ZIP archive
  - `Auto` (default): Intelligent selection based on number and type of sources
