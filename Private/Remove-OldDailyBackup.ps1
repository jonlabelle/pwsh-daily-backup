function Remove-DailyBackupInternal
{
    ...
    {
        Write-Verbose ('New-DailyBackup:Remove-DailyBackupInternal> No qualified backup directories to delete were detected in: {0}' -f $Path) -Verbose:$VerboseEnabled
        return
    }
    ...
    if ($sortedBackupPaths.Count -gt $BackupsToKeep)
    {
        for ($i = 0; $i -lt ($sortedBackupPaths.Count - $BackupsToKeep); $i++)
        {
            ...
            {
                Write-Verbose ('New-DailyBackup:Remove-DailyBackupInternal> Removing old backup directory: {0}' -f $backupPath) -Verbose:$VerboseEnabled
                Remove-ItemAlternative -LiteralPath $backupPath -WhatIf:$WhatIfPreference -Verbose:$VerboseEnabled
                Write-Verbose ('New-DailyBackup:Remove-DailyBackupInternal> Successfully removed: {0}' -f $backupPath) -Verbose:$VerboseEnabled
            }
        }
    }
    else
    {
        Write-Verbose 'New-DailyBackup:Remove-DailyBackupInternal> No surplus daily backups to delete' -Verbose:$VerboseEnabled
    }
}
